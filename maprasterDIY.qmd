...
You will then need to download the shapefiles for France. You can use [GADM](https://gadm.org/formats.html) which has made available national and subnational shapefiles for the world.[Geoboundaries](https://www.geoboundaries.org/) also provide a database of political administrative boundaries.
...
3.  Visualise the elevation in `R` or `Python`.
4.  If working in `R`: use `geom_spatraster_contour_filled` to create a contour map with specific breaks. If working in `Python` use Matplotlib's `contourf` or `contour`.
