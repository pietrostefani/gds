...
The command `plot` does not allowed to add additional data, improve the legend, or add a basemap easily. 
`ggplot2` offers more graphics options:
...
You can get away using `st_distance` and `st_area` to calculate great circle distance by default in R. 
Measurement error is not that bad if you forget to project the data. However, I do not recommend this!
