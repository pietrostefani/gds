...
```{python}
nyc = gpd.read_file('./data/NYC/NYC_tract_ACS2008_12.shp')
```
:::

Make sure the crs is set to `EPSG:4325`:
...
-   Create a contiguity matrix using the queen criterion. If using R, use the `nb2listw()` function, but adding the additional `zero.policy = TRUE` argument to ensure that lists of neighbours with zero length are allowed:
...
```{python}
jp_cities = gpd.read_file("./data/JPN/JPN_core_commuting.shp")
```
:::
...
```{python}
jp_centroids = jp_cities.copy()
jp_centroids.geometry = jp_cities.geometry.centroid
```
:::
