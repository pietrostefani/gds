...
Both `m` and `r` need to be prespecified by the user before running `DBSCAN`. This is a critical point, as their value can influence significantly the final result. Before exploring this in greater depth, let us get a first run at computing `DBSCAN`.

In `Python` we use the following module to compute `DBSCAN` for real data. For more on this, check [here](https://scikit-learn.org/stable/modules/clustering.html).

-   `sklearn.cluster` or popular unsupervised clustering algorithms.
