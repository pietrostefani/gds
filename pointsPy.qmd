...
#listings_gdf.plot(column='neighbourhood', ax=ax, legend=False, cmap='Set1')
...
#cx.add_basemap(ax, crs=listings_gdf.crs.to_string())
#cx.add_basemap(
#    ax, 
#    crs="EPSG:4326",
...
```{python}
# Set up algorithm
algo = DBSCAN(eps=0.10, min_samples=10)
```

We decide to consider a cluster of airbnb with more than 50 airbnbs within 100 metres from them, hence we set the two parameters accordingly. Once ready, we “fit” it to our data, but note that we first need to express the longitude and latitude of our points in metres (see code for that on the side cell). Wait, we already did that when we projected listings.

```{python}
algo.fit(listings_proj[["longitude", "latitude"]])
```
...
```{python}
lbls = pd.Series(algo.labels_, index=listings_proj.index)
```
...
ax.scatter(
    listings_proj.loc[listings_proj.index.difference(noise.index), "longitude"], 
    listings_proj.loc[listings_proj.index.difference(noise.index), "latitude"],
    c="red", 
    linewidth=0
)
# Display the figure
