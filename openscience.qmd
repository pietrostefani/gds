...
We can obtain minimum values for each columns
```{r}
min_values <- sapply(census2021, min) # sapply() is used to apply the min() function to each column of the dataframe, resulting in a vector of minimum values for each column. The min_values vector will contain the minimum value for each column.
...
```{r}
min_value_Europe <- min(census2021$Europe)
...
Similarly, we can restrict the calculations to a single row. Here we obtain standard deviation for the row `E01006518`, which represents a particular LSOA.
```{r}
# Extract the specified row by row name
selected_row <- census2021['E01006518', ]

# Calculate the standard deviation for the entire row
std_dev_row <- sd(selected_row)

# Print the standard deviation for the row
print(std_dev_row)
```
...
[`dplyr`](https://www.tidyverse.org/packages/)
